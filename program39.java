...
class ArrayX
{
    public int LastOccurance(int iArray[], int iNum)
    {
        int iCnt = 0, iNo = 0;

        for(iCnt = iArray.length-1; iCnt >= 0; iCnt--)
        {
            if((iArray[iCnt]) == iNum)
            {
                break;
...
public class program39 {
    public static void main(String[] args) {
        ...
        iRet = Aobj.LastOccurance(iArr, iNumber);

        System.out.println();
        
        if(iRet == -1)
        {
            System.out.println("There is no such a elements");
        }
        else
        {
            System.out.println(iNumber+" Last Occurance is : "+ iRet);
        }
    }    
}
